...
public class DbConnector implements Closeable {
    ...
    public List<Transfer> getHistory(Long id) {
        List<Object> objresultList = null;
        List<Transfer> resultList = new ArrayList<>();
        try {
            System.out.println("\n dbconnector gethistory");
            EntityManager em = emf.createEntityManager();
            TypedQuery<Object> query0 = em.createQuery("SELECT a.history FROM AccountDao a WHERE a.id=:id", Object.class);
            query0.setParameter("id", id);

            objresultList = query0.getResultList();

            //System.out.println(objresultList);
        ...
        for (Object object : objresultList) {
            try {
        ...
        Collections.sort(resultList, new Comparator<Transfer>() {
